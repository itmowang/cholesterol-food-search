...
export default async function handler(
    ...
    console.log(accountId,apiToken);
    

    //   const prompt = `请告诉我关于${foodName}的胆固醇含量和摄入建议。`;
    const messages = [
        { role: "system", content: `你是高级营养顾问，专注提供食物胆固醇含量及摄入建议。` },
        { role: "user", content: `输入变量为${foodName}。输出内容必须满足以下要求：1. 全部书面化表达，禁止使用“好的”、“好”等口语化词语；2. 回答中不包含任何冠词；3. 数据及建议必须权威、准确，无歧义，杜绝错误及混乱；4. 严格依据权威营养学标准，客观中立，必要时提供科学依据；5. 格式简洁明了，仅限于所需信息，无多余修饰词。不能加入思考过程。` },
    ];
